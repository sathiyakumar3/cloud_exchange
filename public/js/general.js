...
function getsiteicon(temptype) {
    switch (temptype) {
        case 'Airport':
            tempicon = "fas fa-plane-departure";
            break;
        case 'Agriculture':
            tempicon = "fas fa-seedling";
            break;
        case 'Apartment':
            tempicon = "far fa-building";
            break;
        case 'Automobile':
            tempicon = "fa fa-car";
            break;
        case 'Bank':
            tempicon = "fas fa-landmark";
            break;
        case 'Church':
            tempicon = "fas fa-church";
            break;
        case 'City':
            tempicon = "fas fa-city";
            break;
        case 'Industry':
            tempicon = "fas fa-industry";
            break;
        case 'Home':
            tempicon = "fas fa-home";
            break;
        case 'Hotel':
            tempicon = "fas fa-hotel";
            break;
        case 'Hospital':
            tempicon = "fas fa-hospital-alt";
            break;
        case 'Kovil':
            tempicon = "fas fa-gopuram";
            break;
        case 'School/College':
            tempicon = "fas fa-school";
            break;
        case 'public':
            tempicon = "fa fa-cloud ";
            break;
        case 'Medical Clinic':
            tempicon = "fas fa-clinic-medical";
            break;
        case 'Mosque':
            tempicon = "fas fa-mosque";
            break;
        case 'Vihara':
            tempicon = "fas fa-vihara";
            break;
        case 'Office':
            tempicon = "fas fa-briefcase";
            break;
        case 'Supermarket':
            tempicon = "fas fa-store-alt";
            break;
        case 'University':
            tempicon = "fas fa-university";
            break;
        default:
            tempicon = "far fa-building"
    }
...
function getdeviceicon(type2) {
    ...
    switch (type2) {
        ...
        case 'vavnawh':
            ...
            break;
        case 'smardtchiller':
            devicon = "fas fa-snowflake";
            break;
        case 'smardtchiller_v2':
            devicon = "fas fa-snowflake";
            break;
        case 'ahu':
            devicon = "fas fa-wind";
...
function uma(t) { $("#content").load("content/uma.html?id=" + t), document.getElementById("idtest").value = t }

function dma(t) { $("#content").load("content/dma.html?id=" + t), document.getElementById("idtest").value = t }

function chat(t) { $("#content").load("content/chat.html?id=" + t), document.getElementById("idtest").value = t }
...
function site(domainid, roler) {
    ...
    document.getElementById("idtest").value = domainid;
    document.getElementById("role1").value = roler
}
...
function open_item(e, n, t, s, l) {
    ...
    null != prev_selec && (document.getElementById(prev_selec).className = "sub-nvi-itm") && (document.getElementById(prev_selec).innerHTML = document.getElementById(prev_selec).innerHTML.replace('<i class="fas fa-spinner fa-spin"></i>', '')), unsubscribe && (unsubscribe(), unsubscribe = null), unsubscribe2 && (unsubscribe2(), unsubscribe2 = null), document.getElementById("idtest").value = n, document.getElementById("domainname").value = t, document.getElementById("role1").value = s, document.getElementById("devname").value = l, document.getElementById("subcripText").innerHTML = l + " - " + " <font color='Orange'>[LOADING]</font> &nbsp;&nbsp;&nbsp; |  &nbsp;&nbsp;&nbsp; " + t + "  &nbsp;&nbsp;&nbsp;";
    document.getElementById("li_" + n).className = "navi-selection", prev_selec = "li_" + n;
    var name = document.getElementById("li_" + n).innerHTML;
    document.getElementById("li_" + n).innerHTML = name + "&nbsp;&nbsp;<i class='fas fa-spinner fa-spin'></i>"
}
...
function badnews(error) { Swal.fire("Warning", String(error), 'error'); }

function goodnews(text) { Swal.fire('Success', text, 'success'); }

function datetimeformat(t) {
    var e = t.toDate().getDate() + "-" + (t.toDate().getMonth() + 1) + "-" + t.toDate().getFullYear() + "  " + t.toDate().getHours() + ":" + t.toDate().getMinutes() + ":" + t.toDate().getSeconds();
    return e
}

function numberWithCommas(x) { x = Math.round(x); return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }
...
function adDevice() {
    ...
    var domain = document.getElementById("changeDomain2").value;
    var publickey = document.getElementById("Pkey").value;
    ...
    db.collection("devices").doc(DeviceId).get().then(function(doc) {
        if (doc.exists) { Swal.fire({ title: "\"" + DeviceId + "\" is already used.", text: "Please use a different ID", dangerMode: !0, }).then((willDelete) => { if (willDelete) { $('#exampleModal').modal('show') } }) } else {
            db.collection("types").doc(dType).collection('datasets').doc('live').get().then(function(doc) {
                ...
                db.collection("devices").doc(DeviceId).set({
                    ...
                    domain: domain,
                    verdict: "You can use this field to note the status of the unit.",
                    verdict_created_on: today,
                    verdict_user: displayName,
                    cloud_status: "",
                    image: 'default'
                }, { merge: !0 })
                ...
                var i;
                Object.keys(nio).forEach((name) => {
                    for (i = 0; i < 10; i++) {
                        var nower = new Date();
                        db.collection("devices").doc(DeviceId).collection('alarms').doc().set({ timestamp: nower, parameter: name })
                    }
                })
            }).then(function() {
                Swal.fire({
                    title: "Please wait.",
                    text: "We are setting up the device for you.",
                    timer: 60000,
                    html: '<h6></h6>.',
                    onBeforeOpen: () => {
                        Swal.showLoading();
                        Swal.getContent().querySelector('h6').textContent = "We are setting up the device for you."
                    },
                })
                unsubscribetemp = db.collection("devices").doc(DeviceId).onSnapshot(function(doc) {
                    var status = doc.data().cloud_status;
                    Swal.getContent().querySelector('h6').textContent = "Creating device in the cloud."
                    if (status != "") {
                        unsubscribetemp();
                        if (status == 'success') { rebuildtree("Congrates!, " + DeviceId + " was created successfully.") } else {
                            db.collection("devices").doc(DeviceId).delete().then(function() {
                                Swal.fire({ type: 'error', title: 'Oops...', text: status, });
                                Swal.fire("Error!", status, "error").then((willDelete) => { if (willDelete) { $('#exampleModal').modal('show') } })
                            })
                        }
                    }
                })
...
function CreateSite() {
    ...
    var today = new Date();
    var user = firebase.auth().currentUser;
    var roles = { admin: !1, developer: !1, member: !1, owner: !0 };
    if (/\s/.test(site_name)) { Swal.fire({ title: "Invalid Site Name.", text: "No Spaces allowed in the site name.", buttons: !0, dangerMode: !0, }).then((willDelete) => { if (willDelete) { $('#CreateSitemodal').modal('show') } }) } else {
        db.collection("domains").doc(site_name).set({ name: site_name, created_on: today, description: site_description, location: site_location, owner: user.uid, owner_email: user.email, owner_name: user.displayName, type: site_Type, cloud_status: "" }).then(function() {
            Swal.fire({
                title: "Please wait.",
                text: "We are setting up the site for you.",
                timer: 60000,
                html: '<h6></h6>.',
                buttons: !1,
                ...
                onBeforeOpen: () => {
                    unsubscribetemp = db.collection("domains").doc(site_name).onSnapshot(function(doc) {
                        var status = doc.data().cloud_status;
                        Swal.getContent().querySelector('h6').textContent = "Request Sent for new site.";
                        if (status != "") {
                            unsubscribetemp();
                            if (status == 'success') {
                                ...
                                db.collection("domains").doc(site_name).delete().then(function() { Swal.fire("Error!", status, "error").then((willDelete) => { if (willDelete) { $('#CreateSitemodal').modal('show') } }) })
                            }
                        }
                    })
                }
...
function deleteSite() {
    ...
    Swal.fire({ title: "You are about to delete : " + sName, text: "All devices in the site will be moved to the owner's sandbox.", icon: 'warning', }).then((willDelete) => {
        if (willDelete) {
            db.collection("domains").doc(sid).delete().then(function() { rebuildtree(sName + " was deleted successfully. ") }).catch(function(error) { badnews(error); })
        }
    })
}
...
function resetdevice() {
    ...
    Swal.fire({ title: 'Do you want to proceed?', text: "[" + device_id + "] will get reset.", icon: 'warning', showCancelButton: !0, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Yes, proceed.' }).then((result) => {
        if (result.value) {
            document.getElementById("DeviceName").value = document.getElementById('formdName').value;
            document.getElementById("DeviceId").value = device_id;
            document.getElementById("dDescription").value = document.getElementById('formdescription').value;
            document.getElementById("dType").value = document.getElementById('formdvtype').value;
            document.getElementById("changeDomain2").value = document.getElementById('changeDomain3').value;
            document.getElementById("Pkey").value = document.getElementById('Pkey2').value;
            document.getElementById("pKey_format").value = document.getElementById('formdvformat').value;
            document.getElementById("range").value = document.getElementById('formintevl').value;
            document.getElementById("range2").value = document.getElementById('mmlog').value;
            console.log(document.getElementById("changeDomain2").value);
            console.log(document.getElementById("Pkey").value);
            console.log(document.getElementById("pKey_format").value);
            console.log(document.getElementById("DeviceName").value);
            console.log(document.getElementById("dDescription").value);
            console.log(document.getElementById("DeviceId").value);

            db.collection("devices").doc(device_id).delete().then(function() { adDevice() }).catch(function(error) { badnews(error); })
        } else { Swal.fire({ title: 'Cancelled!', icon: 'warning', text: 'No changes were made.', showConfirmButton: !0, }) }
    })
}

function googleupdate() {
    firebase.auth().onAuthStateChanged(function(e) { null != e.displayName && null != e.email && null != e.photoURL && db.collection("users").doc(e.uid).set({ name: e.displayName, email: e.email, photoUrl: e.photoURL, emailVerified: e.emailVerified }, { merge: !0 }).then(function() { swal("Synced Successfully!", { icon: "success" }) }).catch(function(e) { swal({ title: "Warning", text: e, icon: "warning", buttons: !0, dangerMode: !0 }) }) })
}
...
function ProfileUpdate() {
    var e = document.getElementById("UserEmailUpdte").value,
        t = document.getElementById("UserNameUpdte").value,
        n = document.getElementById("Userphonnumber").value,
        l = document.getElementById("UserGender").value,

        sys = document.getElementById("sys_use").value,

        d = document.getElementById("UserCountry").value,
        o = document.getElementById("UserDesignation").value,
    ...
    console.log(sys),
        Swal.fire({
            ...
            onBeforeOpen: () => {
                Swal.showLoading();
                Swal.getContent().querySelector('h6').textContent = "Saving...";
            },
        })
